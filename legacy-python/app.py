...
from src.screener import screen_stocks, DEFAULT_SCREENER_SYMBOLS
...
def get_client():
    ...
    if _client is None:
        if DATA_SOURCE == "demo":
            ...
            from src.clients.unusual_whales_client import UnusualWhalesClient
...
def screener_endpoint():
    """Scan a list of tickers for entry opportunities.

    Accepts ?symbols=A,B,C — if absent, uses the default DEFAULT_SCREENER_SYMBOLS.
        ...
        if raw:
            ...
            for s in raw.split(","):
                ...
                if len(syms) >= 50:
                    break
            if not syms:
                syms = list(DEFAULT_SCREENER_SYMBOLS)
        else:
            syms = list(DEFAULT_SCREENER_SYMBOLS)
        return jsonify(screen_stocks(syms))
    except Exception as e:  # noqa: BLE001
        return jsonify({"error": str(e)}), 500
...
def candles_endpoint():
    ...
    try:
        ...
        extras = {}
        try:
            from src.pro_indicators import overnight_high_low, volume_confirmation
